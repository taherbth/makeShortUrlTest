    ...
    <div>
        <Layout>
            <div class="page-header">
                <div><h3 class="font-weight-bold">Active Users</h3></div>
                <div>
                    <router-link to="/institution/manage/deactive-users" class="btn btn-border-primary text-default">View Deactivated Users</router-link>
                    <a href="#" class="btn btn-primary text-white" data-toggle="modal" data-target="#InviteModal">Add New User</a>
                </div>
            </div>
                            ...
                            <tr v-for="(user, key) in allUsers" :key="key">
                                ...
                                <td class="text-right">
                                    <div class="custom-control custom-switch custom-switch-md">
                                        <input type="checkbox" class="custom-control-input" :id="`user-approval-${user.member_type}-${user.id}`" checked data-toggle="modal" data-target="#ActiveDeactiveModal" @change="getUserId(user.id, user.member_type)">
            ...
            <pagination align="center" :show-disabled="true" :data="paginationInfo_allUsers" @pagination-change-page="getActiveUsers">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center">
                        ...
                        <h5 class="font-weight-bold text-uppercase">DO you want to deactivate <br> this user?</h5>
                        <p class="text-sm text-neutral">User will be deactivated from billing cycle dd/mm/yyyy till you choose to reactivate them</p>
                        <button type="button" class="btn btn-primary-text mb-5" @click="DeactiveUser">Deactivate</button>
...
<script>
    ...
    export default {
        ...
        methods: {
            getActiveUsers: function (page=1){
                axios.get(`api/institution/get_all_users?query=&is_active=1&page=${page}`).then(res => {
            ...
            getUserId: function (id, member_type){
                this.deActiveUserId = id
                this.deActiveUserType = member_type
            },
            DeactiveUser: function (){
                axios.post(`api/institution/post_deactivate_user?status=0&member_type=${this.deActiveUserType}&user_id=${this.deActiveUserId}`).then(res => {
        ...
        mounted() {
            ...
            $('#ActiveDeactiveModal').on('hidden.bs.modal', function () {
                if($(`#user-approval-${self.deActiveUserType}-${self.deActiveUserId}`) && $(`#user-approval-${self.deActiveUserType}-${self.deActiveUserId}`)[0]){
                    $(`#user-approval-${self.deActiveUserType}-${self.deActiveUserId}`)[0].checked = true
                }else{
