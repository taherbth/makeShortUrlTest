            ...
            <div v-if="tabActive === 'pending'">
                <div v-if="unrated_courses.length">
                    <response-item v-for="unrated_course in unrated_courses" :key="unrated_course.id"
                        ...
                        :url="`/institution/responses/${unrated_course.id}/pending`" :title="unrated_course.title"
                ...
                <pagination align="center" :show-disabled="true" :data="paginationInfo_unrated" @pagination-change-page="get_institution_unrated_courses_response_wise">
            ...
            <div v-if="tabActive === 'provided'">
                <div v-if="rated_courses.length">
                    <response-item v-for="rated_course in rated_courses" :key="rated_course.id"
                        ...
                        :url="`/institution/responses/${rated_course.id}/provided`" :title="rated_course.title"
                ...
                <pagination align="center" :show-disabled="true" :data="paginationInfo_rated" @pagination-change-page="get_institution_rated_courses_response_wise">
                    <span slot="prev-nav">Prev</span>
	                <span slot="next-nav">Next</span>
                </pagination>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            get_institution_rated_courses_response_wise: function (page = 1){
                axios.get(`/api/institution/get_institution_rated_courses_response_wise?page=${page}`).then(res => {
            ...
            get_institution_unrated_courses_response_wise: function (page = 1){
                axios.get(`/api/institution/get_institution_unrated_courses_response_wise?page=${page}`).then(res => {
        ...
        mounted() {
            this.get_institution_rated_courses_response_wise()
            this.get_institution_unrated_courses_response_wise()
        },
    }
</script>
<style scoped>
