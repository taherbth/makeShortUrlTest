    ...
    export default {
        ...
        methods: {
            getCourcesResponses: function (){
                axios.get(`/api/institution/get_institution_rated_or_non_rated_course_responses/${this.$route.params.id}`).then(res => {
            ...
            checkCourseStatus: function (){
                if(this.courseInfo.ques_rated === 2 && this.$route.params.type === "pending"){
                    this.$router.push(`/institution/responses/${this.$route.params.id}/provided`)
                }
            },
        },
        mounted() {
