                    ...
                    <section :class="[step === 1? '':'d-none']">
                        <div class="card video-description">
                            <div class="card-body">
                                ...
                                <div slot="body">
                                    <vInput title="Video title" name="title" :required="true" :errorMsg="errors.title" />
                                    <div class="form-group">
                                        <label>Add a message or a brief summary about the video <span class="text-danger">*</span></label>
                                        <textarea :class="['form-control', errors.details? 'is-invalid':'']" rows="8" name="details" required></textarea>
                                        <small class="form-text text-danger text-left" v-if="errors.details">{{ errors.details[0] }}</small>
                                    </div>
                                    <vInput title="Topic" name="topic" :required="true" :errorMsg="errors.topic" />
                                    <div class="row">
                    ...
                    <section :class="[step === 3? '':'d-none']">
                        <div class="card finish-upload">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    <p>Set who can watch your video</p>

                                    <div class="custom-control custom-radio mb-4">
                                        <input type="radio" class="custom-control-input" id="public" name="is_public" value="1" @change="setVideoPrivacy" checked required>
                                        <label class="custom-control-label" for="public">Everyone in your class</label>
                                        <div class="feedback text-secondary">Everyone will be able to watch the video.</div>
                                    </div>

                                    <div class="custom-control custom-radio mb-3">
                                        <input type="radio" class="custom-control-input" id="private" name="is_public" value="0" @change="setVideoPrivacy" required>
                                        <label class="custom-control-label" for="private">Private</label>
                                        <div class="feedback text-secondary">
                                            <span>Only specific people you will add can watch this video</span>
                                            <div class="form-group mt-2">
                                                <label class="text-secondary">Select people</label>
                                                <select style="width:100%" class="select-2 form-control" name="students[]" multiple :disabled="is_public==1? true:false" required></select>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="mt-4">
                                    <a href="#" class="btn btn-lg btn-light text-secondary" @click="previousStep">Back</a>
                                    <a href="#" class="btn btn-lg btn-light text-secondary">Cancel</a>
                                    <button class="btn btn-lg btn-primary-text text-white" type="submit" data-draft="1" @click="setDraftStatus">Save As Draft</button>
                                    <button class="btn btn-lg btn-danger text-white" type="submit" data-draft="0" @click="setDraftStatus">Publish</button>
                                </div>
                            </div>
                        </div>
                    </section>
    ...
    export default {
        ...
        methods: {
            ...
            submitForm: function (el){
                ...
                axios.post('/api/facilitator/upload_video_info', data).then(response => {
                    $('#video-uploaded').modal('show')
                }).catch(error => {
                    this.ValidtaeForm(error)
                    this.notifyMsg = this.errors.thumbnail[0]
                    $('.toast').toast('show')
                });
            },
            ...
            uploadVideo: function (el){
                ...
                axios.post('/api/facilitator/upload_video', data, config).then(response => {
        ...
        mounted() {
            ...
            axios.get('api/facilitator/get_students').then((res)=>{
                let data = ''
            ...
            axios.get('/api/facilitator/get_thumbnails').then( res => {
                ...
                if(this.progress === false){
                    this.$router.push('/facilitator/video/upload')
                }
            })

            let self = this
            $("#video-uploaded").on("hide.bs.modal", function(){
                self.$router.push('/facilitator/video/upload')
            });
        },
    }
</script>
