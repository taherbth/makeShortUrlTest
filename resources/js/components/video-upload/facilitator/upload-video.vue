                ...
                <div class="col-lg-6">
                    ...
                    <div class="video-container disable-scrollbars" v-if="tabActive === 'drafts'">
                        <div v-if="drafted_videos.length">
                            <VideoItem v-for="(item, key) in drafted_videos" :key="key" :video_url="`/facilitator/watch/play/${item.id}`" :image="item.thumbnail" :title="item.title" :last_edited="convertUploadDate(item)"
                    ...
                    <div class="video-container disable-scrollbars" v-if="tabActive === 'published'">
                        <div v-if="published_videos.length">
                            <VideoItem v-for="(item, key) in published_videos" :key="key" :video_url="`/facilitator/watch/play/${item.id}`" :image="item.thumbnail" :title="item.title" :last_edited="convertUploadDate(item)"
                                :topic="item.topic" :chapter="`Chapter ${item.chapter}`" :episode="`Episode ${item.episode}`"
                                :rating="item.rating" :views="item.viewes"
                            />
                        </div>
    ...
    export default {
        ...
        methods: {
            SendFileToUpload: function(el){
                ...
                this.$router.push('/facilitator/video/upload/info')
            },
            getDraftedVideos: function (page=1){
                axios.get(`/api/facilitator/get_course_drafted?page=${page}`).then(response => {
            ...
            getPublishedVideos: function (page=1){
                axios.get(`/api/facilitator/get_course_published?page=${page}`).then(response => {
                    this.published_videos = Object.values(response.data.data)
