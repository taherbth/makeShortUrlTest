...
class AuthorityApiWatchController extends Controller
{
    ...
    public function __construct(CourseController $CourseController, CacheController $CacheController){
        ...
        $this->cache_type_institution_get_topics='authority_get_topics';
        $this->cache_type_institution_get_courses='authority_get_courses';
    ...
    public function get_course_detail($id){
        ...
        if(!in_array($id,Authority::return_authority_all_courses()['courses_id']->toArray()))
            return response(['Error, This Course is not for you'],500);

        if($course->publishable_id == Authority::return_id() && $course->publishable_type==$this->CourseController->authority_image)
            $self_course=1;        
...
//            return response(['message'=>'Error, Course is not for you'],404);
    ...
    public function toogle_demo_video(Request $request){
        try{
            $show_demo_video = $request->show_demo_video;
            $updated_data = array();
            if($show_demo_video){
                $updated_data['upload_status'] = '1';
                $updated_data['is_public'] = '1';                
                $updated_data['is_draft'] = '0';
            }else{
                $updated_data['upload_status'] = '0';
                $updated_data['is_public'] = '0';                
                $updated_data['is_draft'] = '1';
            }            
            $course_updated = Course::where([
            ...
            if($course_updated){
                return response(
                    ['message'=> 'Success, demo video view option successfully toggled'], 201
                ); 
            } 
        }catch(\Exception $e){
            return response(['message'=>'Error, can not toggle demo video view option'],422);
        }                
    }
}
