...
class FacilitatorApiResponsesController extends Controller
{
    ...
    public function get_course($id){
        ...
        if(
            ...
            ($course->publishable_type == $this->CourseController->institution_image && $course->publishable_id != Facilitator::find(Facilitator::return_id())->institution_id)
            ||
            ($course->publishable_id != Facilitator::return_id() && $course->publishable_type == $this->CourseController->facilitator_image)
        )
            return "false";
        return $course;
    }

    public function get_facilitator_rated_courses_response_wise(){

        try{
            $facilitator_courses= Facilitator::return_facilitator_uploaded_courses()->orderByDesc('id')->get();
        }catch(\Exception $e){
        ...
        try {
            foreach ($facilitator_courses as $facilitator_course) {
                ...
                $temp_responses = $facilitator_course->student_response_c()->get();
                //return $temp_responses;

                if (empty($facilitator_course->student_response_c()->first())) {
                    $unrated_course[] = $facilitator_course;
                } else {
                    foreach ($temp_responses as $temp_response) {
                        ...
                        if (!empty($temp_response->response_rating_sr()->first())
                            && !in_array($facilitator_course, $unrated_course)
                            && !in_array($facilitator_course, $rated_course) ){
                            ...
                            $facilitator_course->avg_rating = $total_rating;
                            $rated_course[] = $facilitator_course;

                        } else if (empty($temp_response->response_rating_sr()->first())
                            && !in_array($facilitator_course, $unrated_course)) {
                            if (in_array($facilitator_course, $rated_course) ){

                                $rated_course = array_diff($rated_course, [$facilitator_course]);
                                if (isset($facilitator_course->avg_rating)) unset($facilitator_course->avg_rating);
                                $unrated_course[] = $facilitator_course;
                                continue;
                            }
                            $unrated_course[] = $facilitator_course;
                ...
                $facilitator_course->responses_count=count($response_array);
            }
    ...
    public function get_facilitator_unrated_courses_response_wise(){

        try{
            $facilitator_courses= Facilitator::return_facilitator_uploaded_courses()->where('upload_status',1)->orderByDesc('id')->get();
        }catch(\Exception $e){
            return response(['message'=>'Error, Can not retrieve your published courses'],404);
        } //Check  if Courses Exist of facilitator
        ...
        try {
            foreach ($facilitator_courses as $facilitator_course) {
                ...
                $temp_responses = $facilitator_course->student_response_c()->get();
                $facilitator_course->responses_count=count($temp_responses);
    ...
    public function get_facilitator_rated_or_non_rated_course_responses($id){
        $course_students=[];
        $response_count=0;
    ...
    public function post_facilitator_rate_responses(RateResponseRequest $request){
        $response_id_s=array_keys($request->rating);
        $student_id=[];
        $user_id = Facilitator::return_id();
        ...
        foreach ($request->rating as $response_id=>$response_rating) {
            ...
            if ($course->publishable_id != $user_id || $course->publishable_type != $this->CourseController->facilitator_image)
            ...
            if($response_rating!=0){
                Facilitator::find($user_id)->response_rating_f()->create([
        ...
        try{
            Student::where('id',$student_id)->first()
                ->notify(new StudentResponseRatedNotification(
                    $user_id,$request->course_id,1
                ));
        } catch(\Exception $e){
