...
class InstitutionApiDashboardController extends Controller
{
    protected $CacheController, $cache_type_institution_last_published_video ,$institution, $cache_type_institution_progress;
    public function __construct(CacheController $CacheController){
        $this->CacheController=$CacheController;
        $this->cache_type_institution_last_published_video='institution_last_published_video';
        $this->cache_type_institution_progress='institution_progress';
        $this->institution=Auth::guard('api-institution')->user();
    }
    public function get_institution_latest_responses(){

        $all_courses_response=collect(new studentResponses());
        try {
            $courses = Institution::return_institution_uploaded_courses()->get();
            ...
            if(!empty($course->student_response_c()->orderByDesc('id')->first())){
                ...
                foreach($course_std_responses as $course_std_response){
                    ...
                    if(in_array($course_std_response->question_type,[3,5])){
                       ...
                       $student_response = courseQuestionOption::find($course_std_response->student_response);
                       if($student_response){
                            $course_std_response->student_response = $student_response->courseQuestion_option;
                       }
                    }
    ...
    public function get_institution_last_published_video(){
        $cache=$this->CacheController->get_cache( $this->institution->id, $this->institution->email,$this->cache_type_institution_last_published_video);
        if($cache != null)
            return $cache;
        else{
            try{
                $courses=Institution::return_institution_uploaded_courses();
                ...
                if(isset($last_course)){
                    $this->CacheController->make_cache( $this->institution->id, $this->institution->email,$last_course,$this->cache_type_institution_last_published_video);
                    return response($last_course);
                }
                else {
                    $this->CacheController->make_cache( $this->institution->id, $this->institution->email,0,$this->cache_type_institution_last_published_video);
                    return 0;
                }
            }catch(\Exception $e){
                return 0;
    ...
    public function get_institution_statistics()
    {
        $now = Carbon::now();
        ...
        $institution_course_responses=new Collection();

        $institution_courses_id=Institution::return_institution_uploaded_courses()->pluck('id');
        $institution_courses = Course::whereIn('id',$institution_courses_id)->get();
        $GLOBALS["start"]=$weekStartDate;
        $GLOBALS["end"]=$weekEndDate;
        $watch_histories =  watchHistory::whereIn('course_id', $institution_courses_id)
            ->where(function ($q) {
                $q->where([
        ...
        foreach ($institution_courses as $institution_course){
            if($institution_course -> student_response_c()->exists()){
                $responses =$institution_course -> student_response_c()-> get();
                foreach ($responses as $response){
                    if(
                        ...
                        $institution_course_responses->push($response);
                }
            }
        }
        ...
        foreach ($institution_course_responses as $institution_course_response) {
            if ($institution_course_response->created_at >= $institution_course_response->updated_at)
                $institution_course_response->last_watched = $institution_course_response->created_at;
            else
                $institution_course_response->last_watched = $institution_course_response->updated_at;

            $institution_course_response->last_watched_day=$institution_course_response->last_watched->format('l');

        }

        foreach ($all_days as $day){
            $student_responses_count_array[] = $institution_course_responses->where('last_watched_day',$day)->count();
    ...
    public function get_institution_progress(){
        $cache= $this->CacheController->get_cache($this->institution->id,$this->institution->email,$this->cache_type_institution_progress);
        if($cache != null)
            return $cache;
        ...
        try{
            ...
            $all_courses_public=$institution->course_i()->where([['upload_status',1],['is_public',1],['is_draft',0]])->get();
            ...
            $all_courses_private=$institution->course_i()->where([['upload_status',1],['is_public',0],['is_draft',0]]);
            ...
            $students=$institution->student_s();
            ...
            $this->CacheController->make_cache($this->institution->id,$this->institution->email,$progress,$this->cache_type_institution_progress);
            return $progress;
